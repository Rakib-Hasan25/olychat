...
export async function POST(req: NextRequest) {
  try {
    ...
    if (existingChat) {
      ...
      const title =JSON.parse(messages[1]).content||"new chat";
      ...
      console.log("succesfully inserted chat id  ");

      if (newChatError) {
        throw new Error(`Error creating new chat: ${newChatError.message}`);
      }
    }
    ...
    const formattedMessages = messages.map((msg) => {
      ...
      return {
        ...
        message_type: parsedMessage.message_type || "text", // Default to "text" if not provided
        created_at: parsedMessage.timestamp, // Assuming Redis message has a `timestamp`
      };
    });
